...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try { $pdo->exec("ALTER TABLE listings ADD COLUMN location VARCHAR(255) DEFAULT 'Campus'"); } catch (PDOException $e) {}
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-cancel-link { display: block; text-align: center; margin-top: 20px; color: var(--slate); font-weight: 600; text-decoration: none; font-size: 14px; transition: 0.2s;}
        .btn-cancel-link:hover { color: var(--indigo); }
...
<script>
    ...
    // Premium GPS Auto-Detect Logic (100% Intact from user prompt)
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                try {
                    ...
                    status.style.color = "var(--mint)"; // Updated to match light theme token
                } catch (e) {
                    status.textContent = "Could not resolve address. Please type it manually.";
                    status.style.color = "var(--coral)"; // Updated to match light theme token
                }
            }, () => {
