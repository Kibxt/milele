...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            // Check if user is verified (Google users are verified automatically)
            if ($user['is_verified'] == 0) {
                $error = "Please verify your email address before logging in.";
            } else {
                $_SESSION['user_id'] = $user['user_id'];
                header("Location: profile.php"); // Or dashboard.php depending on your setup
                exit();
            }
        } else {
...
<head>
    ...
    <style>
        ...
        .forgot-password { display: block; text-align: right; color: #888; font-size: 0.85rem; margin-top: -10px; margin-bottom: 20px; text-decoration: none;}
        .forgot-password:hover { color: #2DD4BF;}
    </style>
</head>
<body>
